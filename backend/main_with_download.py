...
async def verify_image_endpoint(request: Request, file: UploadFile = File(...)):
    ...
    # 8. Return Full URLs for Frontend
    return {
        "verdict": result.get("verdict"),
        "confidence": result.get("ensemble_probability"),
...
async def verification_page(request: Request, record_id: str):
    ...
    media_html = ""
    # Assuming record['original_url'] is relative path /static/...
    # We construct full URL for display if needed, but relative works in HTML
    display_url = record['original_url']
    
    # NEW: Download Link for Watermarked Version
    # Note: DB stores relative path /static/watermarked/...
